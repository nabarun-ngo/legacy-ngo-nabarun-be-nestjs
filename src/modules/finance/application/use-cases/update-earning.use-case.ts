...
@Injectable()
export class UpdateEarningUseCase
  implements
    IUseCase<{ id: string; dto: UpdateEarningDto; userId: string }, Earning>
{
  constructor(
  ...
  async execute(request: {
    ...
    if (request.dto.status == EarningStatus.RECEIVED) {
      if (!request.dto.accountId) {
        throw new BusinessException(
          "Account ID is required to mark earning as received",
        );
      }
      if (!request.dto.earningDate) {
        throw new BusinessException(
          "Earning Date is required to mark earning as received",
        );
      }

      earning.markAsReceived(
        request.dto.accountId,
        request.dto.earningDate,
        request.userId,
      );
    ...
    const updatedEarning = await this.earningRepository.update(
      request.id,
      earning,
    );
