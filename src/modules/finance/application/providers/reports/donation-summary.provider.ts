...
DonationType,
PaymentMethod,
} from "src/modules/finance/domain/model/donation.model";
import {
DONATION_REPOSITORY,
...
@Injectable()
@ReportProvider()
export class DonationSummaryReportProvider
  implements IReportProvider<{ startDate: Date; endDate: Date }>
{
  ...
  async generate(params: {
    ...
    const endDt = (
      typeof params.endDate === "string"
        ...
        : DateTime.fromJSDate(params.endDate)
    ).setZone("Asia/Kolkata");
    ...
    const isExactFullMonth =
      startDt.day === 1 &&
      endDt.toFormat("yyyy-MM-dd") ===
        startDt.endOf("month").toFormat("yyyy-MM-dd");
  ...
  private async template(request: {
    startDate: Date;
    endDate: Date;
    ...
    const accountWisePaidDonations = groupBy(
      paidDonations,
      (donation) => donation.paidToAccount?.id,
    );
    const memberWisePendingDonations = groupBy(
      pendingDonations.filter((f) => !f.isGuest),
    ...
    for (const donation of paidDonations) {
      paidDonationsData.push({
        ...
        period:
          donation.type == DonationType.REGULAR
        ...
        confirmedOn: formatDate(donation.confirmedOn!, {
          format: "dd/MM/yyyy",
        }),
    ...
    for (const [donorId, donations] of Object.entries(
      memberWisePendingDonations,
    )) {
    ...
    const summaryData = {
      ...
      totalPendingAmount: pendingDonations.reduce(
        (acc, d) => acc + d.amount,
        0,
      ),
    ...
    summarySheet
      ...
      .setCell(
        11,
        1,
        "Total Paid Amount in " + monthName,
      ...
      .setCell(
        13,
        2,
        summaryData.totalPendingAmount,
        ExcelStyles.rupeeAmountStyle,
      )
    ...
    for (const acc of accountWisePaidDonationsData) {
      summarySheet.setCell(
        summaryCurrentRow,
        1,
        `${acc.id} - ${acc.accountHolder}`,
        {
    ...
    summarySheet.setCell(
      summaryCurrentRow + 2,
      1,
      `NOTE: This report is based on Donation ${request.on === "paidOn" ? "Payment Date" : "Confirmation Date"}.`,
