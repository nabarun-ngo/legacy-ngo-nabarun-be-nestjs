...
} from "src/modules/finance/domain/repositories/account.repository.interface";
import {
DONATION_REPOSITORY,
type IDonationRepository,
} from "src/modules/finance/domain/repositories/donation.repository.interface";
import {
EARNING_REPOSITORY,
type IEarningRepository,
} from "src/modules/finance/domain/repositories/earning.repository.interface";
import {
EXPENSE_REPOSITORY,
type IExpenseRepository,
} from "src/modules/finance/domain/repositories/expense.repository.interface";
...
@Injectable()
@ReportProvider()
export class AuditReportProvider
  implements IReportProvider<{ financialYear: string }>
{
  ...
  private async template(request: { financialYear: string }): Promise<Buffer> {
    ...
    const [startYear, endYear] = request.financialYear
      .split("-")
      .map((y) => parseInt(y));
    ...
    const endDate = DateTime.fromObject({ year: endYear, month: 3, day: 31 })
      .endOf("day")
      .toJSDate();
    ...
    const receivedEarnings = earnings.filter(
      (e) => e.status === EarningStatus.RECEIVED,
    );
    ...
    data
      ...
      .setCell(
        ...
        ExcelStyles.totalRowLabelStyle,
      )
    ...
    data
      ...
      .setCell(
        expSectionHeaderRow,
        1,
        "Consolidated Expenditures Breakdown",
        ExcelStyles.sectionHeaderStyle,
      )
      .setCell(
        expSectionHeaderRow + 1,
        1,
        "Expense Category",
        ExcelStyles.labelBoldStyle,
      )
    ...
    refData.donationType.forEach((item) =>
      donationTypeMap.set(item.KEY, item.VALUE),
    );

    const donationStatusMap = new Map<string, string>();
    refData.donationStatus.forEach((item) =>
      donationStatusMap.set(item.KEY, item.VALUE),
    );

    const paymentMethodMap = new Map<string, string>();
    refData.paymentMethod.forEach((item) =>
      paymentMethodMap.set(item.KEY, item.VALUE),
    );
    ...
    const donationRows = donations.map((d) => {
      ...
      return {
        ...
        period:
          d.type == "REGULAR"
            ? `${safeFormatDate(d.startDate)} - ${safeFormatDate(d.endDate)}`
            : "",
        ...
        paymentMethod:
          paymentMethodMap.get(d.paymentMethod ?? "") || d.paymentMethod || "-",
        paidUsingUPI:
          upiTypeMap.get(d.paidUsingUPI ?? "") || d.paidUsingUPI || "-",
    ...
    refData.earn_categories.forEach((item) =>
      earningTypeMap.set(item.KEY, item.VALUE),
    );

    const earningStatusMap = new Map<string, string>();
    refData.earn_status.forEach((item) =>
      earningStatusMap.set(item.KEY, item.VALUE),
    );
    ...
    refData.exp_categories.forEach((item) =>
      expenseTypeMap.set(item.KEY, item.VALUE),
    );

    const expenseStatusMap = new Map<string, string>();
    refData.exp_status.forEach((item) =>
      expenseStatusMap.set(item.KEY, item.VALUE),
    );
    ...
    const expenseRows = expenses.map((e) => {
      ...
      return {
        expenseId: e.id,
        category:
          expenseTypeMap.get(e.referenceType ?? "") || e.referenceType || "-",
        name: e.name,
