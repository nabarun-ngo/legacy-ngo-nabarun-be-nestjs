...
export class AccountController {
  ...
  async listSelfAccountTransactions(
    ...
    const result = await this.accountService.listTransactions(
      accountId,
      {
        pageIndex,
        pageSize,
  ...
  async transferAmountSelf(
    ...
    const result = await this.accountService.transferAmount(
      accountId,
      dto,
      user?.profile_id,
    );
  ...
  @ApiAutoResponse(AccountRefDataDto, {
    wrapInSuccessResponse: true,
    description: "Donation reference data retrieved successfully",
  })
