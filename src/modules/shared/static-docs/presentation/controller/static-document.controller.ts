...
export class StaticDocsController {
  ...
  @ApiAutoResponse(StaticDocumentDto, {
    status: 200,
    description: "User guides fetched successfully",
    isArray: true,
    wrapInSuccessResponse: true,
  })
  async getUserGuides(): Promise<SuccessResponse<StaticDocumentDto[]>> {
    return new SuccessResponse(await this.statDocsService.getUserGuides());
  }
  ...
  async getPolicies(): Promise<SuccessResponse<StaticDocumentDto[]>> {
    return new SuccessResponse(await this.statDocsService.getPolicyDocs());
  }

  @Get("app-links")
  @ApiOperation({ summary: "Get app links" })
  @RequirePermissions("read:static_docs")
  @ApiAutoResponse(KeyValueDto, {
    status: 200,
    description: "App links fetched successfully",
    isArray: true,
    wrapInSuccessResponse: true,
  })
  @ApiQuery({
