...
} from "../automatic-task-handlers/automatic-task-handler.interface";
...
export class AutomaticTaskRegistryService implements OnApplicationBootstrap {
  ...
  private autoRegister() {
    ...
    providers.forEach((wrapper) => {
      ...
      if (handlerName) {
        this.logger.log(
          `Discovered @AutomaticTaskHandler: ${handlerName} (${instance.constructor.name})`,
        );
    ...
    this.logger.log(
      `Auto-registration complete. Successfully registered ${registeredCount} task handlers.`,
    );
  }
  ...
  private register(handler: IAutomaticTaskHandler, handlerName: string) {
    if (this.handlers.has(handlerName)) {
      this.logger.warn(
        `Task handler ${handlerName} is already registered. Overwriting.`,
      );
    }
