...
export class ActivityEventsHandler {
    ...
    async handleActivityCompletedEvent(event: ActivityCompletedEvent) {
        this.logger.log(`Handling ActivityCompletedEvent for activity: ${event.activity.id}`);
        try {
            await this.jobProcessingService.addJob(
                JobName.TriggerReportJobEvent,
                {
                    reportCode: 'ACTIVITY_REPORT',
                    params: { activityId: event.activity.id },
                }
            );
            this.logger.log(`Successfully enqueued ACTIVITY_REPORT generation for activity: ${event.activity.id}`);
        } catch (error) {
            this.logger.error(`Failed to enqueue ACTIVITY_REPORT generation for activity: ${event.activity.id}`, error);
        }
    }
}
