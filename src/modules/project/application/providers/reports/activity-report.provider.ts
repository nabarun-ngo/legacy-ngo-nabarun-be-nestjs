...
import {
IReportProvider,
ReportGeneratedData,ReportProvider
...
@Injectable()
@ReportProvider()
export class ActivityReportProvider
  implements IReportProvider<{ activityId: string }>
{
  ...
  async generate(params: { activityId: string }): Promise<ReportGeneratedData> {
    ...
    if (!activity) {
      throw new NotFoundException(
        `Activity with ID ${params.activityId} not found`,
      );
    }
  ...
  private async template(request: { activity: Activity }): Promise<Buffer> {
    const subActivities = await this.activityRepository.findByParentActivityId(
      request.activity.id,
    );
    ...
    sheet
      ...
      .setCell(11, 1, "Name", ExcelStyles.labelBoldStyle)
      .setCell(11, 2, activity.name, ExcelStyles.labelStyle)
      .setCell(12, 1, "Type", ExcelStyles.labelBoldStyle)
      .setCell(12, 2, activity.type, ExcelStyles.labelStyle)
      .setCell(13, 1, "Scale", ExcelStyles.labelBoldStyle)
      .setCell(13, 2, activity.scale, ExcelStyles.labelStyle)
      .setCell(14, 1, "Status", ExcelStyles.labelBoldStyle)
      .setCell(14, 2, activity.status, ExcelStyles.labelStyle)
      .setCell(15, 1, "Priority", ExcelStyles.labelBoldStyle)
      .setCell(15, 2, activity.priority, ExcelStyles.labelStyle)
      .setCell(16, 1, "Venue", ExcelStyles.labelBoldStyle)
      .setCell(16, 2, activity.venue || "N/A", ExcelStyles.labelStyle)
      .setCell(17, 1, "Location", ExcelStyles.labelBoldStyle)
      .setCell(17, 2, activity.location || "N/A", ExcelStyles.labelStyle)

      .mergeCells(19, 1, 19, 3)
      .setCell(19, 1, "Participation & Metrics", ExcelStyles.sectionHeaderStyle)
      .setCell(20, 1, "Expected Participants", ExcelStyles.labelBoldStyle)
      .setCell(
        20,
        2,
        activity.expectedParticipants || 0,
        ExcelStyles.labelStyle,
      )
      .setCell(21, 1, "Actual Participants", ExcelStyles.labelBoldStyle)
      .setCell(21, 2, activity.actualParticipants || 0, ExcelStyles.labelStyle)

      .mergeCells(23, 1, 23, 3)
      .setCell(23, 1, "Cost Details", ExcelStyles.sectionHeaderStyle)
      .setCell(24, 1, "Estimated Cost", ExcelStyles.labelBoldStyle)
      .setCell(24, 2, activity.estimatedCost || 0, ExcelStyles.rupeeAmountStyle)
      .setCell(25, 1, "Actual Cost", ExcelStyles.labelBoldStyle)
      .setCell(25, 2, activity.actualCost || 0, ExcelStyles.rupeeAmountStyle);

    const expenses = await this.financeQuery.getActivityExpenses(activity.id);
    const validExpenses = expenses.filter(
      (e) => e.status !== ExpenseStatus.REJECTED,
    );
    const settledExpenses = validExpenses.filter(
      (e) => e.status === ExpenseStatus.SETTLED,
    );
    const unsettledExpenses = validExpenses.filter(
      (e) => e.status !== ExpenseStatus.SETTLED,
    );
    const settledAmount = settledExpenses.reduce((sum, e) => sum + e.amount, 0);
    const unsettledAmount = unsettledExpenses.reduce(
      (sum, e) => sum + e.amount,
      0,
    );
    const totalExpenseAmount = settledAmount + unsettledAmount;

    const donations = await this.financeQuery.getActivityDonations(activity.id);
    const validDonations = donations.filter(
      (d) => d.status !== DonationStatus.CANCELLED,
    );
    const paidDonations = validDonations.filter(
      (d) => d.status === DonationStatus.PAID,
    );
    const unpaidDonations = validDonations.filter(
      (d) => d.status !== DonationStatus.PAID,
    );
    ...
    sheet
      ...
      .setCell(28, 1, "Total Expenses", ExcelStyles.labelBoldStyle)
      .setCell(28, 2, totalExpenseAmount, ExcelStyles.rupeeAmountStyle)
      .setCell(29, 1, "Settled Expenses", ExcelStyles.labelBoldStyle)
      .setCell(29, 2, settledAmount, ExcelStyles.rupeeAmountStyle)
      .setCell(30, 1, "Unsettled Expenses", ExcelStyles.labelBoldStyle)
      .setCell(30, 2, unsettledAmount, ExcelStyles.rupeeAmountStyle)
      .setCell(31, 1, "Total Donations", ExcelStyles.labelBoldStyle)
      .setCell(31, 2, totalDonationAmount, ExcelStyles.rupeeAmountStyle)
      .setCell(32, 1, "Paid Donations", ExcelStyles.labelBoldStyle)
      .setCell(32, 2, paidAmount, ExcelStyles.rupeeAmountStyle)
      .setCell(33, 1, "Unpaid Donations", ExcelStyles.labelBoldStyle)
      .setCell(33, 2, unpaidAmount, ExcelStyles.rupeeAmountStyle);
    ...
    if (subActivities.length > 0) {
      sheet
        ...
        .setCell(
          ...
          ExcelStyles.sectionHeaderStyle,
        )
    ...
    const safeDate = (date?: Date | null) =>
      date ? formatDate(date, { format: "dd/MM/yyyy" }) : "-";
      ...
      .addSheet({
        ...
        columns: [
          ...
          { header: "Currency", key: "currency" },
          {
            header: "Amount",
            key: "amount",
            style: ExcelStyles.rupeeAmountStyle,
          },
