...
export class ReportInfraMapper {
  static toDomain(p: ReportPersistence): Report | null {
    ...
    return new Report(
      ...
      p.requestedBy
        ? {
            id: p.requestedBy.id,
            firstName: p.requestedBy.firstName,
            lastName: p.requestedBy.lastName,
          }
        : p.requestedById
          ? { id: p.requestedById }
          : undefined,
      ...
      p.approvedBy
        ? {
            id: p.approvedBy.id,
            firstName: p.approvedBy.firstName,
            lastName: p.approvedBy.lastName,
          }
        : p.approvedById
          ? { id: p.approvedById }
          : undefined,
  ...
  static toCreatePersistence(domain: Report): Prisma.ReportCreateInput {
    return {
      ...
      requestedBy:
        domain.requestedBy?.id && domain.requestedBy.id !== "system"
      ...
      approvedBy:
        domain.approvedBy?.id && domain.approvedBy.id !== "system"
