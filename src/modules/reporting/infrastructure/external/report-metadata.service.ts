...
export class ReportMetadataService {
  ...
  async getReportDefinations(): Promise<ReportDefination[]> {
    ...
    const definations = parseKeyValueConfigs(
      keyValueConfigs["REPORT_DEFINATIONS"].value,
    );
    return definations.map((reportDefination) => {
      return {
        ...
        requiresApproval: reportDefination.getAttribute<boolean>(
          "IS_APPROVAL_REQUIRED",
        ),
        approverRoles:
          reportDefination.getAttribute<string[]>("APPROVER_ROLES") || [],
        visibleToRoles:
          reportDefination.getAttribute<string[]>("VISIBLE_TO_ROLES") || [],
      } as ReportDefination;
    });
  }
  ...
  async getReportDefination(reportCode: string): Promise<ReportDefination> {
    ...
    const reportDefination = definations.find(
      (def) => def.reportCode === reportCode,
    );
    if (!reportDefination) {
