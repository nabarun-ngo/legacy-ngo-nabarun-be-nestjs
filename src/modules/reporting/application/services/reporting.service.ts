...
import {
fieldAttributeDomainToDto,
mapToAdditionalFields,
} from "src/shared/utilities/additional-field.util";
...
export class ReportingService {
  ...
  async generateReport<T extends Record<string, any>>(
    reportCode: string,
    params: T,
    authUser: AuthUser,
  ) {
    ...
    if (!provider) {
      throw new NotFoundException(
        `Report provider for ${reportCode} not found`,
      );
    }
    const defination =
      await this.metadataService.getReportDefination(reportCode);
    if (!defination) {
      throw new NotFoundException(
        `Report defination for ${reportCode} not found`,
      );
    }
    if (defination.approverRoles && defination.approverRoles.length > 0) {
      const hasApproverRole = authUser.user_roles?.some((role) =>
        defination.approverRoles?.includes(role),
      );
      if (!hasApproverRole) {
        throw new BadRequestException(
          `You do not have the required role to generate this report. Required: ${defination.approverRoles.join(", ")}`,
        );
      }
    }
    ...
    const result = await this.processAndSaveReportDocument(
      report,
      generatedData,
      authUser.profile_id!,
    );
  ...
  async regenerateReport(reportId: string, authUser: AuthUser) {
    ...
    if (report.approvers && report.approvers.length > 0) {
      const hasApproverRole = authUser.user_roles?.some((role) =>
        report.approvers?.includes(role),
      );
      if (!hasApproverRole) {
        throw new BadRequestException(
          `You do not have the required role to generate this report. Required: ${report.approvers.join(", ")}`,
        );
      }
    }
    ...
    if (!provider) {
      throw new NotFoundException(
        `Report provider for ${report.reportCode} not found`,
      );
    }
  ...
  private async processAndSaveReportDocument(
    ...
    if (!isRegenerate) {
      if (report.needApproval) {
        ...
        const workflow = await this.workflowFacade.startWorkflow({
          type: "REPORT_REVIEW",
          data: {
  ...
  async updateStatus(
    ...
    if (status === ReportStatus.APPROVED) {
      if (report.approvers && report.approvers.length > 0) {
        ...
        if (!hasApproverRole) {
          throw new BadRequestException(
            `You do not have the required role to update status to ${status}. Required: ${report.approvers.join(", ")}`,
          );
        }
      }
      report.markApproved(authUserId);
  ...
  async getReportInputFields(reportCode: string) {
    const additionalFieldDef =
      await this.metadataService.getAdditionalFieldDef();
    const provider = this.registry.getProvider(reportCode);
    if (!provider) {
      throw new NotFoundException(
        `Report provider for ${reportCode} not found`,
